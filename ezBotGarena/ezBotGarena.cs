...
namespace ezBotGarena
{
    internal class ezBotGarena
    {
        ...
        public ezBotGarena(string username, string password, string reg, string path, int threadid, QueueTypes QueueType, string LoLVersion)
        {
            ipath = path;
            ...
            {
                case "TH":
                    connection.Connect(username, password, Region.TH, LoLVersion);
                    break;
                case "SGMY":
                    connection.Connect(username, password, Region.SGMY, LoLVersion);
                    break;
                case "TW":
                    connection.Connect(username, password, Region.TW, LoLVersion);
                    break;
                case "PH":
                    connection.Connect(username, password, Region.PH, LoLVersion);
                    break;
                case "VN":
                    connection.Connect(username, password, Region.VN, LoLVersion);
                    break;
                case "ID":
                    connection.Connect(username, password, Region.ID, LoLVersion);
                    break;
            }
        }
                ...
                switch (game.GameState)
                {
                    ...
                    case "CHAMP_SELECT":
                        ...
                        if (firstTimeInLobby)
                        {
                            ...
                            Tools.ConsoleMessage("In Champion Select");
                            ...
                            if (queueType != QueueTypes.ARAM)
                            {
                                ...
                                else if (Program.firstChampionPick == "RANDOM")
                                {
                                    int Spell1;
                                    int Spell2;
                                    if (!Program.randomSpell)
                                ...
                                //Select second chamion
                                if (Program.secondChampionPick != "" && Program.secondChampionPick != "RANDOM")
                                {
                                    int Spell1;
                                    int Spell2;
                                    if (!Program.randomSpell)
                                ...
                                else if (Program.secondChampionPick == "RANDOM")
                                {
                    ...
                    case "POST_CHAMP_SELECT":
                        ...
                        if (firstTimeInPostChampSelect)
                        {
                            ...
                            Tools.ConsoleMessage("Post Champion Select");
                        }
                        break;
                    case "IN_QUEUE":
                        Tools.ConsoleMessage("In Queue");
                        break;
                    case "TERMINATED":
                    ...
                    case "JOINING_CHAMP_SELECT":
                        ...
                        {
                            Tools.ConsoleMessage("Accepted qwueue");
        ...
        private async void EnterQueue()
        {
            ...
            else
            {
                ...
                {
                    Tools.ConsoleMessage("Waiting out leaver buster: " + m_leaverBustedPenalty / 1000 / (float)60 + " minutes!");
        ...
        private void connection_OnLogin(object sender, string username, string ipAddress)
        {
            new Thread((ThreadStart)(async () =>
            {
                ...
                Tools.ConsoleMessage("Logged in as: " + loginPacket.AllSummonerData.Summoner.Name + " - level " + loginPacket.AllSummonerData.SummonerLevel.Level);
